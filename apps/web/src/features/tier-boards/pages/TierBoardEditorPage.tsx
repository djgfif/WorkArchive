...
function CardImage({ imageUrl, title }: { imageUrl: string; title: string }) {
            ...
            <Menu position="bottom-end" shadow="md">
              ...
              <Menu.Dropdown>
                <Menu.Item
                  onClick={async () => {
                    const duplicated =
                      await tierBoardService.duplicateBoard(activeBoardId);
                    navigate(`/tier-boards/${duplicated.id}`);
                  }}
                >
                  보드 복제
                </Menu.Item>
                <Menu.Item onClick={() => navigate('/tier-boards')}>
                  목록으로 이동
                </Menu.Item>
              </Menu.Dropdown>
            </Menu>
              ...
              <Tabs className={cn(css.addTabs)} defaultValue="upload">
                ...
                <Tabs.Panel pt="md" value="upload">
                  <Stack className={cn(css.addForm)} gap="sm">
                    <input
                      ...
                      hidden
                      onChange={(event) =>
                        handleUploadFile(event.currentTarget.files?.[0] ?? null)
                      }
                      ref={uploadInputRef}
                      type="file"
                    />
                ...
                <Tabs.Panel pt="md" value="url">
                  <Stack className={cn(css.addForm)} gap="sm">
                    ...
                    <AppButton
                      ...
                      fullWidth
                      onClick={() => void handleCreateUrlCard()}
                      tone="primary"
                      type="button"
                    >
                      카드 추가
                ...
                <Tabs.Panel pt="md" value="text">
                  <Stack className={cn(css.addForm)} gap="sm">
                    ...
                    <AppButton
                      fullWidth
                      onClick={() => void handleCreateTextCard()}
                      tone="primary"
                      type="button"
                    >
                      카드 추가
                      ...
                      {filteredWorks.map((work) => (
                        ...
                        >
                          <Group align="center" gap="sm" wrap="nowrap">
                            ...
                            <AppButton
                              onClick={() => void handleImportWork(work.id)}
                              tone="secondary"
                              type="button"
                            >
                              Snapshot 추가
              ...
              <Select
                ...
                onChange={(value) =>
                  ...
                  void tierBoardService
                    .applyLaneTemplate(activeBoardId, value)
                    .then(loadState)
                }
