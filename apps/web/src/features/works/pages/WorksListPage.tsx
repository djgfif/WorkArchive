          ...
          <Box>
            ...
            {!isLoading && !isTrashScope && totalActiveCount > 0 && (
              <Box className={cn(css.libraryPageMeta)}>
                <span>작품 <strong>{totalActiveCount}개</strong></span>
                {statusCounts.in_progress > 0 && (
                  <>
                    <span className={cn(css.libraryPageDot)} />
                    <span>진행 중 <strong>{statusCounts.in_progress}개</strong></span>
                  </>
                )}
                {statusCounts.completed > 0 && (
                  <>
                    <span className={cn(css.libraryPageDot)} />
                    <span>완료 <strong>{statusCounts.completed}개</strong></span>
                  </>
                )}
                {statusCounts.planned > 0 &&
                  ...
                  <>
                    <span className={cn(css.libraryPageDot)} />
                    <span>볼 예정 <strong>{statusCounts.planned}개</strong></span>
                  </>
                )}
              </Box>
            )}
            {!isLoading && isTrashScope && totalDeletedCount > 0 && (
              <Box className={cn(css.libraryPageMeta)}>
                <span>삭제된 작품 <strong>{totalDeletedCount}개</strong></span>
              </Box>
            )}
          </Box>
          {!isTrashScope && (
