...
export function CommunityReflectionPage() {
  ...
  const loadFeed = useCallback(async () => {
    ...
    try {
      const response = await fetchCommunityReflections(sort);
  ...
  async function handlePublish() {
    ...
    try {
      const published = await publishCommunityReflection(
  ...
  async function handleLoadMore() {
    ...
    try {
      const response = await fetchCommunityReflections(sort, nextCursor);
      if (requestId !== feedRequestId.current) return;
      setPosts((current) => {
  ...
  async function handleReaction(post: CommunityPostView) {
    ...
    try {
      await setCommunityReflectionReaction(post.id, post.viewerHasReacted);
      setPosts((current) =>
        current.map((entry) =>
          entry.id === post.id
            ? {
  ...
  async function handleDelete(postId: string) {
    ...
    try {
      await deleteCommunityReflection(postId);
  ...
  async function handleReport(postId: string, reason: CommunityReportReason) {
    ...
    try {
      await reportCommunityReflection(postId, reason);
