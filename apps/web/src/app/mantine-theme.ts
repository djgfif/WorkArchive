...
export const appCssVariablesResolver: CSSVariablesResolver = (_theme) => ({
  variables: {
    ...
    '--app-font-figure':
      '"JetBrains Mono", "Pretendard Variable", Pretendard, "Apple SD Gothic Neo", ui-monospace, monospace',
  ...
  dark: {
    ...
    '--mantine-color-placeholder': 'var(--wa-text-muted)',
  ...
  light: {
    ...
    '--wa-text-muted':
      '#6e6e7c' /* 가장 밝은 subtle surface 대비 ≈4.6:1 (AA) */,
    ...
    '--mantine-color-dimmed': 'var(--wa-text-muted)',
    '--mantine-color-placeholder': 'var(--wa-text-muted)',
    ...
    '--app-accent-secondary': 'var(--wa-accent-ink)',
    '--app-accent-warm':
      '#b45309' /* 별점 전용 앰버 — 화이트 대비 ≈4.3:1 (AA), 보편적 평점 골드 (라이트) */,
    '--app-accent-teal': 'var(--wa-accent-teal)',
    '--app-accent-rose': 'var(--wa-accent-rose)',
    ...
    Input: {
      styles: {
        input: {
          ...
          transition:
            'border-color var(--wa-motion-fast), box-shadow var(--wa-motion-fast)',
        },
      },
    },
    ...
    Menu: {
      ...
      styles: {
        ...
        item: {
          ...
          transition:
            'background var(--wa-motion-fast), color var(--wa-motion-fast)',
        },
        label: {
          color: 'var(--app-text-muted)',
          fontSize: 'var(--app-type-meta)',
