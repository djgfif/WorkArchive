...
function rectanglesOverlap(first: BoundingRectangle, second: BoundingRectangle) {
  return (
...
test('opens the command palette and runs a library search', async ({ page }) => {
  await gotoApp(page, '/');
  ...
  await expect(page).toHaveURL(/\/works\?q=%EC%9D%80%ED%95%98%EC%B2%A0%EB%8F%84$/);
...
test('keeps mobile add-work save actions from covering first fields', async ({
  ...
  const layoutMetrics = await page.evaluate(() => {
    ...
    const genreButton = buttons.find((button) =>
      button.textContent?.includes('장르 선택'),
    );
    const saveFooter = saveButton?.closest('[class*="addWorkSaveFooter"]');
    const toRectangle = (element: Element | undefined) => {
      if (!element) {
        return null;
      }
    ...
    return {
      footerPosition:
        saveFooter instanceof HTMLElement
          ...
          : null,
      genreSelectorBox: toRectangle(genreButton),
  ...
  expect(layoutMetrics.genreSelectorBox).not.toBeNull();
  expect(
    rectanglesOverlap(
      layoutMetrics.saveButtonBox!,
      layoutMetrics.genreSelectorBox!,
    ),
  ).toBe(false);
...
test('lets mobile users navigate core routes from the drawer', async ({
  ...
  await expect(page.getByRole('heading', { name: '오늘 펼쳐볼 작품' })).toBeVisible();
  ...
  await expect(mobileNavigation.getByRole('link', { name: '작품' })).toBeVisible();
  await expect(
...
test('shows quick-add source coverage and keeps direct-add fallback visible', async ({
  ...
  await expect(candidateButton.getByText('출처 2개', { exact: true })).toBeVisible();
...
test('keeps direct add usable when quick-add search fails', async ({ page }) => {
  await mockImportProviderStatus(page);
  await page.route('**/api/imports/search**', async (route) => {
    await route.fulfill({
      body: JSON.stringify({
  ...
  await expect(
    searchFailureAlert.getByText('서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요.'),
  ).toBeVisible();
  await expect(
