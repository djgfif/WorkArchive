...
Last reviewed: 2026-08-25 Community alpha implementation and focused service tests.
...
| `community_post`                                                           | satisfied   | not_exposed | satisfied   | not_exposed | not_exposed | Public reads filter to published rows; create assigns the authenticated author from the guard; delete uses both post ID and author ID; moderator hide/restore requires role authorization and an audit row.                                                                                                                                                  | `apps/api/test/community.service.spec.ts` covers published-only reads without raw user IDs, explicit publication fields, owner-scoped soft delete, role rejection, and atomic hide/audit.                                                                                                                                      |
| `community_reaction`                                                       | not_exposed | satisfied   | satisfied   | not_exposed | not_exposed | Reactions are addressed only through the authenticated user's unique `(postId, userId)` row and visible post scope.                                                                                                                                                                                                                                          | `apps/api/test/community.service.spec.ts` covers idempotent current-user create/delete selection and published-post scope.                                                                                                                                                                                                     |
