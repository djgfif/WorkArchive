...
| GitHub controls               | Branch protection, required checks, CodeQL, Dependabot, secret scanning, push protection                                | C     | GitHub API verification on 2026-08-31 confirms strict `master` protection, required `verify`/`integration`/`CodeQL`, secret scanning, push protection, Dependabot security updates, and zero open Dependabot alerts. CodeQL has 8 open alerts whose current-master paths were statically triaged as `not_actionable`; their GitHub disposition remains open. | Release owner review of the 8 CodeQL dispositions; record any dismissal rationale or waiver owner/expiry without weakening required checks.                                                                                  |
...
- Date and timezone: 2026-08-31 KST (repository and GitHub control refresh; policy-specific QA rows retain individual dates)
- Commit SHA: 681fdf9ac4291f921c9dc4cecf01908f6d498e9c (`master` merge commit; GitHub validate and CodeQL passed)
- Host or environment: local WSL repository gates plus GitHub-hosted required CI; beta host evidence remains open
...
- `npm run security:public`: PASS — public readiness and secret leak checks passed locally and in `master` CI (2026-08-31)
- `npm run check:docs-links`: PASS — all markdown local links valid locally and in `master` CI (2026-08-31)
...
- `npm run lint`: PASS — no ESLint errors across workspaces locally and in `master` CI (2026-08-31)
- `npm run typecheck`: PASS — no TypeScript errors locally and in `master` CI (2026-08-31)
- `npm run test`: PASS — API 97 suites / 801 tests, web 86 files / 534 tests, shared-types 2 files / 6 tests, plus site/server tests passed locally and in `master` CI (2026-08-31)
- `npm run build`: PASS — shared-types, API, site, and web production builds passed; largest JavaScript chunk 620,419 bytes against the 650,000-byte limit (2026-08-31)
...
- Required checks: VERIFIED through the GitHub API on 2026-08-31 — strict `verify`, `integration`, and `CodeQL`; PR #76 merged only after all three succeeded
- CodeQL result: PASS for PR #76 and `master` commit `681fdf9ac4291f921c9dc4cecf01908f6d498e9c` on 2026-08-31; 8 open alerts (6 High, 2 Medium) align to that commit and were statically triaged as `not_actionable`, while GitHub disposition remains open for release-owner review
- Dependabot enabled: VERIFIED through the GitHub API on 2026-08-31 — security updates enabled, npm + github-actions configuration present, and 0 open alerts
...
- Secret scanning enabled: ENABLED through the GitHub API on 2026-08-31
- Push protection enabled: ENABLED through the GitHub API on 2026-08-31
